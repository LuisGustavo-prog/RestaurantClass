...
def controller_create_order(data: PostUserSchema):
    try:
        Order(table_number=data.table_number, waiter_id=data.waiter_id, main_course=data.main_course, drink=data.drink, starter=data.starter, dessert=data.dessert)
        return data
...
def controller_update_order(data: PutUserSchema):
    try:
        return Order.put_order(table_number=data.table_number, item_id=data.item_id, new_main_course=data.new_main_course, new_drink=data.new_drink, new_starter=data.new_starter, new_dessert=data.new_dessert)
    except ValueError as e:
        return {'error': True, 'message': str(e)}
